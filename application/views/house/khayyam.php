  ...
  <div style="overflow-x:auto;">
    <div class="container py-5 h-200">
      ...
      <div class="d-flex justify-content-center mb-2 img-container">
        <img src="./assets/img/banner_img_01.jpg" alt="Al-Khayyam Advisers">
    </div>
                  <h6 class="mb-2 text-success" style="font-size: 25px;">Al-Khayyam</h6>
...
<script>
    function showContactInfoModal() {
        Swal.fire({
            ...
            html:
                '<input id="phone" class="swal2-input" placeholder="Phone number" type="number">' +
                '<input id="facebook" class="swal2-input" placeholder="Facebook link">',
            showCancelButton: true,
            confirmButtonText: 'Save',
            preConfirm: () => {
                const phone = parseInt(Swal.getPopup().querySelector('#phone').value);
            ...
            if (result.isConfirmed) {
                ...
                // Display a success message with an OK button
                Swal.fire({
                    title: 'Success',
                    text: 'House information saved successfully!',
                    icon: 'success',
...
?>
                              ...
                              <tr>
                                 <th>Facebook:</th>
                                 <td>www.facebook.com/<br>alrazi</td>
                              </tr>
                              <tr>
                                 <th>Total Members:</th>
                                 <td>231</td>
          ...
          <div class="col-lg-8">
            <div class="card mb-4">
              <div class="card-body">
               ...
               <h6 class="mb-2 text-success" style="font-size: 25px;">House Announcements</h6>
...
<?php if ($this->session->userdata('type') == 'adviser') { ?>
    ...
    <div class="status-post">
        ...
        <div class="status-content">
            <textarea id="status-text" placeholder="House Announcement..."></textarea>
        </div>
        <div class="status-actions">
            <button id="add-announcement-button" class="add-button" onclick="confirmPostStatus()">Post</button>
        </div>
    </div>
    ...
    <script>
        function addStatus() {
            ...
            var statusElement = createStatusElement(statusText);
            var statusContainer = document.getElementById("status-container");
            statusContainer.insertBefore(statusElement, statusContainer.firstChild); // Insert at the beginning
            document.getElementById("status-text").value = "";
        }

        function confirmRemove(removeButton) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    removeStatus(removeButton);
                    Swal.fire("Announcement Removed", "", "success");
                }
            });
        }
        ...
        function createStatusElement(statusText) {
            ...
            statusElement.className = "status-post";
    ...
    var statusText = document.getElementById("status-text").value; // Get the text from the input field

    Swal.fire({
        title: "Confirm Post",
        text: "Are you sure you want to post this House Announcement?\n\n",   // Include the statusText in the confirmation message
        text:statusText,
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "Save",
        ...
        if (result.isConfirmed) {
            addStatus(statusText); // Pass the statusText to the addStatus function
            Swal.fire("Announcement Posted", "", "success");
        }
    });
}

function addStatus(statusText) {
    // Create a new status element with the provided statusText
    var statusElement = createStatusElement(statusText);
